...
class OFDMAChannel(object):
    def __init__(self, 
        ...
        self.PathLoss, self.pos_receivers = self.PathLoss(no_transmitters, no_receivers, 
    ...
    def PathLoss(self, no_transmitters, no_receivers, 
        ...
        PathLoss = np.power(distance_row, -eff_path) 
        return PathLoss, pos_receivers 
    def channel(self, no_samples):
        ...
        for i in range(no_samples):
            PathLoss = 10**(-3)*self.PathLoss*np.sqrt(1/2)
            rand = randn(*channel_size)+randn(*channel_size)*1j 
            channel = rand
            ChannelMRT = LA.norm(channel, axis = 3)
            ChannelMRC = LA.norm(ChannelMRT, axis = -1)
            # PathLossReshape = PathLoss[...,None]
            # PathLossReshape = PathLoss.reshape(self.no_transmitters, self.no_receivers, 
            #                                    self.no_subcarriers, 1, 1)
            ChannelGain = (ChannelMRC**2)*PathLoss
            ChannelMRCs.append(ChannelMRC)
            ChannelGains.append(ChannelGain)
        ChannelMRCs = np.array(ChannelMRCs)
        ChannelGains = np.array(ChannelGains)
        print(ChannelGains)
        return ChannelMRCs, ChannelGains
